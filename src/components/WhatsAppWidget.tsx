...
type Step = "closed" | "bubble" | "form" | "success";
...
export function WhatsAppWidget() {
  ...
  const FloatingBtn = () => (
    <button
      ...
      className="group relative flex items-center justify-center w-14 h-14 rounded-full shadow-lg transition-transform hover:scale-110 active:scale-95"
    ...
    >
      ...
      {/* Tooltip */}
      <span className="absolute right-16 whitespace-nowrap rounded-full bg-white text-xs font-medium text-gray-800 px-3 py-1 shadow opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none">
        Chat with us
      </span>
    </button>
  );
  ...
  const Modal = () => (
    <div className="fixed inset-0 z-[9998] flex items-end sm:items-center justify-center p-4">
      ...
      />
      {/* Card */}
      <div className="relative z-10 w-full max-w-md rounded-3xl bg-[#fdf8f5] shadow-2xl overflow-hidden">
        {/* Header */}
        <div className="flex items-center gap-3 px-6 py-5" style={{ backgroundColor: "#25D366" }}>
          ...
          <div>
            <p className="font-semibold text-white text-sm">Dr. Gamze Eren Clinic</p>
            <p className="text-white/80 text-xs">We typically reply within a few hours</p>
          </div>
          <button
            onClick={() => setStep("bubble")}
        ...
        <form onSubmit={handleSubmit} className="px-6 py-6 grid gap-4">
          <p className="text-sm text-gray-500 -mt-1">Share a few details and we'll reach out on WhatsApp.</p>
          ...
          <div>
            ...
            <input
              ...
              className="w-full rounded-xl border border-gray-200 bg-white px-4 py-2.5 text-sm text-gray-800 outline-none focus:border-[#25D366] transition"
            />
          </div>
          ...
          <div>
            <label className="block text-xs font-semibold uppercase tracking-wider text-gray-500 mb-1">Procedure of interest *</label>
            <select
              required
          ...
          <div>
            <label className="block text-xs font-semibold uppercase tracking-wider text-gray-500 mb-1">Anything else? (optional)</label>
            <textarea
              rows={3}
              value={form.message}
              onChange={e => setForm(f => ({ ...f, message: e.target.value }))}
              placeholder="Timeline, questions, concerns..."
              className="w-full rounded-xl border border-gray-200 bg-white px-4 py-2.5 text-sm text-gray-800 outline-none focus:border-[#25D366] transition resize-none"
            />
          </div>
  ...
  const Success = () => (
    <div className="fixed inset-0 z-[9998] flex items-end sm:items-center justify-center p-4">
      ...
      <div className="relative z-10 w-full max-w-md rounded-3xl bg-[#fdf8f5] shadow-2xl p-8 text-center">
        ...
        <p className="mt-2 text-sm text-gray-500">We've received your details and will reach out on WhatsApp shortly.</p>
        <a
