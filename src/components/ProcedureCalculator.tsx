...
export function ProcedureCalculator({ labels }: Props) {
  ...
  const availableInCategory = PROCEDURES.filter(
    (p) =>
      p.category === pickCategory && !selected.find((s) => s.id === p.id)
  );
  ...
  function discountLabel(index: number): string {
    if (index === 0) return labels.fullPrice;
    return `25% ${labels.off}`;
  }
        ...
        <div className="mt-6 glass-card rounded-2xl p-6 sm:p-8">
          ...
          <div className="space-y-3">
            {sortedSelected.map((proc, i) => (
              <div
                key={proc.id}
                className="flex items-center justify-between gap-4 rounded-xl bg-white/50 px-4 py-3"
              >
                ...
                <div className="text-right shrink-0">
                  <p className="text-sm font-semibold text-[var(--color-ink)]">
                    €{Math.round(proc.low * (i === 0 ? 1 : 0.75)).toLocaleString()} – €{Math.round(proc.high * (i === 0 ? 1 : 0.75)).toLocaleString()}
                  </p>
                </div>
                <button
                  onClick={() => remove(proc.id)}
                  className="ml-2 text-[var(--color-cocoa)]/40 hover:text-rose-400 transition text-lg leading-none"
