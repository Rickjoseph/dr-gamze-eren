export const post = {
  slug: "rinoplasti-iyilesme-sureci-hafta-hafta",
  title: "Rinoplasti Sonrası İyileşme: Hafta Hafta Neler Beklenir?",
  excerpt: "Rinoplasti sonrası iyileşme süreci nasıl ilerler? Dr. Gamze Eren, ameliyat sonrası ilk 24 saatten nihai sonuca kadar hafta hafta nelerin yaşandığını adım adım anlatıyor.",
  date: "2026-05-20",
  category: "Rinoplasti",
  readTime: "6 dk okuma",
  locale: "tr",
  image: "/img/blog/rhinoplasty.jpg",
  imageAlt: "Rinoplasti iyileşme süreci — Dr. Gamze Eren kliniği İstanbul",
  body: `
Rinoplasti, en ince teknik detayları gerektiren estetik operasyonların başında gelir. Pek çok hasta ameliyatı planlarken iyileşme sürecini merak eder: Ne zaman işe dönebilirim? Morluklar ne zaman geçer? Sonuç ne zaman netleşir?

Bu rehberde Dr. Gamze Eren, ameliyat sonrası ilk 24 saatten on ikinci aya kadar sürecin nasıl ilerlediğini adım adım aktarıyor.

## İlk 24 Saat

Ameliyattan çıktığınızda burnunuzda bir alçı veya splint, olası burun tıkacı olacaktır. Kafanızı yastıkla yüksekte tutmanız şişliği azaltır. Bu ilk gün yorgunluk, hafif bulantı ve baş ağrısı normaldir; bunlar anestezinin etkisiyle geçer.

Evde rahatlayın, sıcak içeceklerden kaçının, öne eğilmeyin.

## 1. Hafta

İlk birkaç günde şişlik ve morluk en yoğun seviyededir. Gözler çevresinde ve yanaklarda belirgin olabilir — bu tamamen beklenen bir durumdur ve ameliyatın "başarısız olduğu" anlamına gelmez.

Alçı ya da splint genellikle 7–10. günde çıkarılır. Bu an çoğu hasta için heyecan vericidir; ancak burun hâlâ şiş ve sert görünebilir. Sabırlı olun: bu ilk şekil, kalıcı sonuç değildir.

Dikkat edilmesi gerekenler:
- Gözlük kullanmayın (burnunuza baskı yapar)
- Güneş gözlüğü için alternatif çözüm isteyin
- Burnunuzu temizlerken nazik olun, aletleri derin sokmayın
- Hapşırma veya öksürme geliyorsa ağzınızı açık tutun

## 2–3. Hafta

Morlukların büyük kısmı geçmiş olur; makyaj ile kapatılabilecek düzeye iner. Sosyal hayata dönmek artık mümkündür. Yüz görüntülü toplantı veya iş ortamı için rahat hisseden hastalar genellikle 10–14. günde döner.

Egzersiz, spor, yüzme gibi aktiviteler hâlâ yasaktır.

## 1. Ay

Şişliğin yaklaşık yüzde sekseni geçmiştir. Burun artık görece normal görünür. Çevrenizdeki insanların büyük çoğunluğu fark etmez bile. Çok dikkatli bakıldığında hafif bir dolgunluk hissedilebilir — bu normaldir.

Bu dönemde güneşten korunmak önemlidir; bronzlaşma doğal iyileşme izlerine karışabilir.

## 3–6. Ay

Burun giderek incelmektedir. İnce detaylar — burun ucu, kanatlar, köprü — daha belirgin hale gelir. Nefes almak da iyileşme sürecinde düzelir; septum tamiri yapılmışsa bu dönemde nefes açıklığını hissedersiniz.

## 12. Ay: Nihai Sonuç

Rinoplastide kalıcı sonuç tam anlamıyla bir yılda netleşir. Özellikle burun ucu cildi kalın olan hastalarda bu süre biraz daha uzayabilir. On ikinci ayda çekilen fotoğraf, ameliyat öncesiyle karşılaştırıldığında farkın tüm boyutunu ortaya koyar.

## Dr. Gamze Eren ile Takip Sürecine Dair

Kliniğimizde ameliyat sonrası kontroller düzenli aralıklarla yapılır: 1. hafta, 1. ay, 3. ay ve 12. ay. Uluslararası hastalarımızla video konsültasyonla da iletişimi sürdürüyoruz.

İyileşme sürecindeki her sorunuz için kliniğimize ulaşabilirsiniz. Hiçbir endişe küçük değildir.
  \`,
};
