import type { Treatment } from "./types";

// NOTE: clinical content, please have Dr. Gamze Eren review before launch.
export const abdominoplasty: Treatment = {
  slug: "abdominoplasty",
  category: "body",
  image: "/img/service-body.jpg",
  imageAlt: "Body contouring at Dr. Gamze Eren's clinic",
  content: {
    en: {
      metaTitle: "Abdominoplasty (Tummy Tuck) in Istanbul, Dr. Gamze Eren",
      metaDescription:
        "Tummy tuck to remove loose skin and repair abdominal muscle for a flatter, firmer midline. Dr. Gamze Eren, Istanbul.",
      title: "Abdominoplasty (Tummy Tuck)",
      tagline: "A flatter, firmer abdomen, skin and muscle restored.",
      summary:
        "Abdominoplasty removes loose, excess skin and tightens the abdominal muscles that pregnancy or weight change can separate, restoring a flatter, firmer midline that diet and exercise often can't achieve alone.",
      sections: [
        {
          heading: "Who it helps",
          body: "After pregnancy or significant weight loss, the abdominal skin can hang loose and the muscles can separate (a condition called diastasis), leaving a bulge no amount of exercise resolves. A tummy tuck addresses both the skin and the muscle wall.\n\nDr. Eren tailors the approach, from a mini-tummy tuck to a full or extended procedure, to what your anatomy needs, and often combines it with liposuction for a smoother contour.",
        },
        {
          heading: "The procedure",
          body: "Excess skin is removed through an incision low on the abdomen, designed to sit within the bikini line, and the separated muscles are repaired with internal sutures to rebuild a firm core. The belly button is preserved and repositioned naturally.\n\nIt is performed under general anaesthesia in an accredited hospital, usually with one or two nights' stay.",
        },
        {
          heading: "Recovery & results",
          body: "You'll wear a supportive garment and walk gently from day one, standing fully upright after about a week or two. Most people return to desk work in two to three weeks and exercise gradually after six weeks.\n\nThe result is a flatter, stronger abdomen and a scar that fades over time, hidden under underwear.",
        },
      ],
      faqs: [
        {
          q: "Where is the scar?",
          a: "The main incision sits low across the abdomen, designed to be hidden by underwear or a bikini. It fades over the following year.",
        },
        {
          q: "Is it the same as liposuction?",
          a: "No, liposuction removes fat, while a tummy tuck also removes loose skin and repairs separated muscle. They are often combined.",
        },
      ],
    },
    tr: {
      metaTitle: "İstanbul'da Karın Germe (Abdominoplasti), Dr. Gamze Eren",
      metaDescription:
        "Gevşek deriyi alan ve karın kasını onaran, daha düz ve sıkı bir orta hat için karın germe. Dr. Gamze Eren, İstanbul.",
      title: "Karın Germe (Abdominoplasti)",
      tagline: "Daha düz, sıkı bir karın, deri ve kas onarılır.",
      summary:
        "Karın germe, gevşek ve fazla deriyi alır ve gebelik ya da kilo değişiminin ayırabileceği karın kaslarını sıkılaştırır; diyet ve egzersizin çoğu zaman tek başına sağlayamadığı daha düz ve sıkı bir orta hat kazandırır.",
      sections: [
        {
          heading: "Kimlere uygundur",
          body: "Gebelik veya belirgin kilo kaybından sonra karın derisi sarkabilir ve kaslar ayrılabilir (diastazis denir); bu, hiçbir egzersizin gidermediği bir çıkıntı bırakır. Karın germe hem deriyi hem de kas duvarını ele alır.\n\nDr. Eren yaklaşımı —mini karın germeden tam veya genişletilmiş işleme kadar— anatominizin ihtiyacına göre uyarlar ve daha pürüzsüz bir kontur için sıkça liposuction ile birleştirir.",
        },
        {
          heading: "Ameliyat",
          body: "Fazla deri, bikini çizgisinin içinde kalacak şekilde tasarlanan karnın alt kısmındaki bir kesiden alınır ve ayrılan kaslar iç dikişlerle onarılarak sıkı bir merkez yeniden oluşturulur. Göbek deliği korunur ve doğal biçimde yeniden konumlandırılır.\n\nAkredite bir hastanede genel anestezi altında, genellikle bir-iki gece yatışla yapılır.",
        },
        {
          heading: "İyileşme ve sonuçlar",
          body: "Destekleyici bir giysi giyer ve ilk günden itibaren hafifçe yürürsünüz; yaklaşık bir-iki hafta sonra tam dik durabilirsiniz. Çoğu kişi iki-üç haftada masa başı işine döner, altı haftadan sonra kademeli olarak egzersize başlar.\n\nSonuç daha düz, daha güçlü bir karın ve zamanla soluklaşan, iç çamaşırı altında gizli bir izdir.",
        },
      ],
      faqs: [
        {
          q: "İz nerede kalır?",
          a: "Ana kesi karnın alt kısmında, iç çamaşırı veya bikini ile gizlenecek şekilde yer alır. İzleyen yıl içinde soluklaşır.",
        },
        {
          q: "Liposuction ile aynı mı?",
          a: "Hayır, liposuction yağı alır; karın germe ise ayrıca gevşek deriyi alır ve ayrılan kası onarır. Çoğu zaman birlikte yapılırlar.",
        },
      ],
    },
    ru: {
      metaTitle: "Абдоминопластика (подтяжка живота) в Стамбуле — Dr. Gamze Eren",
      metaDescription:
        "Подтяжка живота: удаление дряблой кожи и восстановление мышц для более плоской, упругой средней линии. Доктор Гамзе Эрен, Стамбул.",
      title: "Абдоминопластика (подтяжка живота)",
      tagline: "Более плоский, упругий живот — кожа и мышцы восстановлены.",
      summary:
        "Абдоминопластика удаляет избыток дряблой кожи и подтягивает мышцы живота, которые могут разойтись после беременности или изменения веса, восстанавливая более плоскую и упругую среднюю линию, которой часто не добиться одними диетой и спортом.",
      sections: [
        {
          heading: "Кому подходит",
          body: "После беременности или значительного похудения кожа живота может обвисать, а мышцы расходиться (состояние, называемое диастазом), оставляя выпуклость, которую не убрать упражнениями. Подтяжка живота решает и кожу, и мышечную стенку.\n\nДоктор Эрен подбирает подход — от мини-абдоминопластики до полной или расширенной операции — под потребности вашей анатомии и часто сочетает его с липосакцией для более гладкого контура.",
        },
        {
          heading: "Операция",
          body: "Избыток кожи удаляется через разрез внизу живота, спроектированный так, чтобы располагаться в зоне белья, а разошедшиеся мышцы восстанавливаются внутренними швами для крепкого кора. Пупок сохраняется и естественно перемещается.\n\nВыполняется под общим наркозом в аккредитованной больнице, обычно с одной-двумя ночёвками.",
        },
        {
          heading: "Восстановление и результат",
          body: "Вы будете носить поддерживающее бельё и понемногу ходить с первого дня, полностью выпрямляясь примерно через одну-две недели. Большинство возвращается к офисной работе за две-три недели, к спорту — постепенно после шести недель.\n\nРезультат — более плоский, крепкий живот и рубец, который со временем бледнеет и скрыт под бельём.",
        },
      ],
      faqs: [
        {
          q: "Где находится рубец?",
          a: "Основной разрез проходит внизу живота и спроектирован так, чтобы его скрывали бельё или бикини. Он бледнеет в течение следующего года.",
        },
        {
          q: "Это то же, что липосакция?",
          a: "Нет — липосакция удаляет жир, а подтяжка живота ещё убирает дряблую кожу и восстанавливает разошедшиеся мышцы. Их часто сочетают.",
        },
      ],
    },
        de: {
      metaTitle: "Bauchdeckenstraffung (Abdominoplastik) in Istanbul – Dr. Gamze Eren",
      metaDescription:
        "Bauchdeckenstraffung zur Entfernung überschüssiger Haut und Straffung der Bauchmuskulatur. Dr. Gamze Eren, Istanbul.",
      title: "Bauchdeckenstraffung (Abdominoplastik)",
      tagline: "Einen flachen, strafferen Bauch – auch wenn Sport und Diät es nicht erreichen.",
      summary:
        "Eine Bauchdeckenstraffung entfernt überschüssige Haut und Fett und strafft die darunterliegenden Muskeln für einen flacheren, definierten Bauch. Besonders wirksam nach Schwangerschaft oder erheblichem Gewichtsverlust.",
      sections: [
        {
          heading: "Was sie leistet",
          body: "Wenn Haut und Muskeln nach Schwangerschaft oder Gewichtsverlust erschlafft sind, kann Sport das Problem nicht vollständig beheben. Eine Abdominoplastik strafft die Muskeln, entfernt überschüssige Haut und formt die Taille neu – für ein saubereres, proportionaleres Ergebnis.\n\nSie wird häufig mit Liposuktion kombiniert und ist ein Kernelement des Mommy Makeovers.",
        },
        {
          heading: "Der Eingriff",
          body: "Durch einen diskreten, horizontal verlaufenden Schnitt (meist unterhalb der Bikinizone) werden Haut und Fett entfernt, die Bauchmuskeln gestrafft und der Nabel neu positioniert. Eine Mini-Bauchdeckenstraffung ist für weniger ausgedehnte Korrekturen unterhalb des Nabels möglich.\n\nDurchgeführt unter Vollnarkose mit einer bis zwei Übernachtungen.",
        },
        {
          heading: "Erholung & Ergebnisse",
          body: "Zwei bis vier Wochen werden Aktivitäten eingeschränkt; ein Kompressionskleidungsstück wird einige Wochen getragen. Die meisten kehren innerhalb von zwei Wochen zu leichter Arbeit zurück, volles Training nach sechs bis acht Wochen.\n\nDas Ergebnis ist dauerhaft, solange das Gewicht stabil bleibt; zukünftige Schwangerschaften können das Ergebnis beeinflussen.",
        },
      ],
      faqs: [
        {
          q: "Was ist der Unterschied zwischen Bauchdeckenstraffung und Liposuktion?",
          a: "Liposuktion entfernt Fett; eine Bauchdeckenstraffung entfernt überschüssige Haut und strafft Muskeln. Häufig werden beide kombiniert für ein optimales Ergebnis.",
        },
        {
          q: "Wann ist der Narbe sichtbar?",
          a: "Die Narbe liegt tief im Bikinibereich und wird sorgfältig platziert. Sie verblasst über 12–18 Monate erheblich.",
        },
      ],
    },
  },
};
